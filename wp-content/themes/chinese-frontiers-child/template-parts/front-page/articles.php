  ...
  <section class="sec-articles container xxl:max-w-fhd px-4" data-aos="fade">
    ...
    <div class="grid xl:grid-cols-4 md:grid-cols-2 xl:gap-x-4 gap-4">
      <?php foreach ($articles as $key => $article) :
        $obj = $article['artykul'];
        $delay = 300 + (($key + 1) * 50); ?>
        <article class="sec-articles__block container" data-aos="fade" data-aos-anchor=".sec-articles" data-aos-delay="<?php echo $delay; ?>">
            ...
            if ($tags) : ?>
              <ul class="sec-articles__block--tags">
                <?php foreach ($tags as $tag) :
                  ...
                  <li class="<?php echo ($text_color) ? 'border border-' . $text_color . '' : 'border-gray'; ?>"><a href="<?php echo get_term_link($tag->term_id); ?>"><?php echo $tag->name; ?></a>
                  </li>
                <?php endforeach; ?>
              </ul>
            <?php endif; ?>
    ...
    <?php if (wp_count_posts()->publish > 4) : ?>
      <div class="flex justify-center pt-11">
        <a href="<?php echo get_permalink(get_option('page_for_posts')); ?>" class="cta cta-blue"><?php _e('Więcej', 'zelmer'); ?></a>
      </div>
    <?php endif; ?>
  </section>
<?php endif; ?>